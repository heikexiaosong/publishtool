...
/**
 * Properties调用store方法保存时，会把文件中原来的注释给冲掉，而且顺序会发生错乱，
 * 网上查找解决方案时，有人遇到了相同的问题，并给出了解决方案
 *
...
public class SafeProperties extends Properties {
    ...
    public static final SafeProperties INSTANCE = build("config.properties");
    ...
    public SafeProperties(String filename) {
        this.filename = filename;
    }

    public static synchronized SafeProperties build(String filename) {
        SafeProperties safeProperties = new SafeProperties(filename);
        try ( FileInputStream ins = new FileInputStream(filename) ) {
            safeProperties.load(ins);
        } catch ( Exception e) {
            e.printStackTrace();
        }
    ...
    public synchronized void store(OutputStream out, String header) throws IOException {
        BufferedWriter awriter = new BufferedWriter(new OutputStreamWriter(out, "UTF8"));
        if (header != null)
