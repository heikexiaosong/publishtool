...
public class FXMLCrawlerController {
    ...
    private void loadSkus() throws Exception {
        ...
        if ( taskQueue.isEmpty() ) {
            ...
            try {
                List<BrandInfo> brandInfos = SQLExecutor.executeQueryBeanList("select * from BRAND_INFO where FLAG is null or FLAG <> 'X' order by SKUNUM desc limit 1", BrandInfo.class);
